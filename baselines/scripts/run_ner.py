...
def main():
    ...
    if data_args.dataset_config_name is None: 
        data_args.dataset_config_name = data_args.task_name
    source_lang=data_args.task_name
    data_args.task_name = data_args.dataset_config_name
    print("task--------------------",data_args.dataset_config_name)
    ...
    # Metrics
    metric = load_metric("seqeval",cache_dir=model_args.cache_dir+"/"+data_args.task_name)
    ...
    if training_args.do_predict:
        ...
        if data_args.all_predict:
            ...
            lang=data_args.dataset_config_name
            predictions, labels, metrics = trainer.predict(predict_dataset, metric_key_prefix="predict")
            predictions = np.argmax(predictions, axis=2)
